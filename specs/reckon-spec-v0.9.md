...
**Spec version:** 0.9 — agent loop defined by SKILL.md + standalone TypeScript primitives
...
- **Indexer/relayer**: Builder A writes it; Builder B's agent's primitive scripts subscribe to its outputs (FillRecorded events on Base, MongoDB documents)
...
- Registration UI: a small page on the dashboard that takes a desired label, the solver's address, signs a registration intent, and submits to the indexer. Indexer validates and writes to MongoDB
...
- **`FillRegistry.recordFill(orderHash, fillerNamehash, swapper, inputAmount, outputAmount, eboTolerance, fillBlock, challengeDeadline)`** is the state-modifying function. Called by **Reckon's off-chain indexer/relayer**, not by the UniswapX reactor. The indexer:
...
- **Trigger**: subscribed to `FillRecorded` events from `FillRegistry` on Base mainnet (via the indexer); on each event, run the loop
...
- **If the indexer/relayer is offline**: fills are not recorded → challenges cannot be opened. Mitigation: redundant indexer instances; permissionless backfill is future work
...
- **Multi-output orders**: rejected in v1, indexer skips with warning
...
**Off-chain code count:** indexer/relayer (~400 LoC), CCIP-Read gateway (~150 LoC), challenger agent (SKILL.md + orchestrator + 5 primitives, ~500 LoC total), dashboard (~600 LoC), KeeperHub skill pack (~300 LoC), AXL helper package (~150 LoC). All TypeScript except the AXL Go binary.
...
### Phase 1 — Core contracts + view-only validator + indexer + benchmark + ENS wildcard resolver + MongoDB + agent scaffold (Week 1)
...
Day 6-7: **Build the indexer/relayer** (~10 hours). TypeScript + viem + Bun. Subscribes to UniswapX `PriorityOrderReactor.Fill` events. Handles subname registration. Skips multi-output orders. Batches fills to 0G Galileo Storage Log.
...
End of Week 1 milestone: A's contracts deploy on Anvil-forked Base mainnet. A's `ReckonWildcardResolver` deployment plan ready (actual mainnet deploy in Phase 2). A's indexer subscribes to real `PriorityOrderReactor` events on Anvil. B's agent has SKILL.md + 4 working primitives (`coordinate.ts` scaffolded), and the orchestrator can drive a fill through to a "would-decide-to-challenge" decision.
...
Day 14: End-to-end Foundry test against Anvil-forked Base mainnet (real `PriorityOrderReactor` → ReckonValidator → indexer → FillRegistry → Challenger → SolverBondVault slash → RoyaltyDistributor → MongoDB reputation update → CCIP-Read query). Target 90%+ coverage.
...
Day 13: **0G Storage Log integration** — implement batching service inside the indexer (Builder A's component, but Builder B coordinates on the 0G Storage Log usage). Upload batch files via `indexer.upload()`, capture root hash, emit `FillBatchAnchored`.
...
Day 15-17: **Deploy `ChallengerNFT.sol` on 0G Galileo**, forking `github.com/0gfoundation/0g-agent-nft`. `Transfer` events consumed by indexer/relayer to update `OwnerRegistry` on Base + MongoDB.
...
Day 24: **Configure indexer for real Base mainnet** (one config line change).
...
**1. Indexer/relayer reliability (HIGH severity, MEDIUM probability).** The relayer is the hinge of the entire system. Mitigation: redundant indexer instances; idempotent operations.
...
**11. Multi-output order edge case (LOW severity, MEDIUM probability).** Reject in v1, indexer skips with warning.
